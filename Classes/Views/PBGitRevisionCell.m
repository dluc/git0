...
+ (NSArray *)laneColors
{
	...
	if (!laneColors) {
		...
		for (size_t i = 0; i < colorCount; ++i) {
			NSColor *newColor = [NSColor colorWithCalibratedHue:(segment * (float)i) saturation:0.7f brightness:0.8f alpha:1.0f];
...
- (void)drawTriangleInRect:(NSRect)r sign:(char)sign
{
	...
	NSPoint top;
	if (sign == '<')
		top.x = round(r.origin.x) + 10 * (CGFloat)c + 4;
	else {
		top.x = round(r.origin.x) + 10 * (CGFloat)c - 4;
		columnWidth *= -1;
	}
...
- (void)drawRect:(NSRect)dirtyRect
{
	...
	if (cellInfo && ![controller hasNonlinearPath]) {
		float pathWidth = 10 + COLUMN_WIDTH * (float)cellInfo.numColumns;
...
- (void)layout
{
	...
	if (cellInfo) {
		...
		if (!controller.hasNonlinearPath) {
			pathWidth = 10 + COLUMN_WIDTH * (float)cellInfo.numColumns;
		}
...
- (int)indexAtX:(CGFloat)x
{
	...
	float pathWidth = 0;
	if (cellInfo && ![controller hasNonlinearPath])
		pathWidth = 10 + 10 * (float)cellInfo.numColumns;
